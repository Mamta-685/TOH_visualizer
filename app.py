...
@app.route("/solve")
def solve():
    n = int(request.args.get("n")) #User input comes from URL query
    ...
    moves = []
    toh(n, "A", "B", "C", state, moves) #call the recursion

    return jsonify({ # this sends JSON to browser (just the http response )
        "n": n,
        "moves": moves
    })
